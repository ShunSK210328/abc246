...
struct Problem {
    ...
    void solve() {
        int N, Ax, Ay, Bx, By;
        read(cin, N, Ax, Ay, Bx, By);
        --Ax, --Ay, --Bx, --By;
        vs S(N);
        cin >> S;
        const array<int, 4> dx = {-1, -1, 1, 1};
        const array<int, 4> dy = {-1, 1, -1, 1};
        vector D(N, vvi(N, vi(4, INF)));
        vector visited(N, vvb(N, vb(4)));
        deque<tuple<int, int, int>> dq;

        auto can_not_move = [&](int x, int y) -> bool { return x < 0 || x >= N || y < 0 || y >= N || S[x][y] == '#'; };

        rep(i, 0, 4) {
            D[Ax][Ay][i] = 1;
            dq.emplace_back(Ax, Ay, i);
        }
        while (dq.size()) {
            auto [cx, cy, cdir] = dq.front();
            dq.pop_front();
            if (cx == Bx && cy == By) {
                cout << D[cx][cy][cdir] << NL;
                return;
            }
            if (visited[cx][cy][cdir]) continue;
            visited[cx][cy][cdir] = true;
            rep(i, 0, 4) {
                int nx = cx + dx[i], ny = cy + dy[i];
                if (can_not_move(nx, ny)) continue;
                if (i == cdir) {
                    if (chmin(D[nx][ny][i], D[cx][cy][cdir])) {
                        dq.emplace_front(nx, ny, i);
                    }
                } else {
                    if (chmin(D[nx][ny][i], D[cx][cy][cdir] + 1)) {
                        dq.emplace_back(nx, ny, i);
                    }
                }
            }
        }
